...
public class lexeme {
	...
	public int charClass; // 다음 들어오는 글자가 숫자면 0, 글자면 1, +,-,*,/이면 2, =,<,>,!이면 3, ;이면 4, { or } 이면 5, comma 이면  6 , whiteSpace이면 7, ( or ) 이면 8, " 이면 9 ,나머지는 -1
	...
	public void getChar() {
		...
		if (this.nextChar >= 48 && this.nextChar <= 57) {
			this.charClass = 0;
		} else if ((this.nextChar >= 65 && this.nextChar <= 90) || (this.nextChar >= 97 && this.nextChar <= 122)) { // 다음에 들어온 글자가 letter이면
			this.charClass = 1;
